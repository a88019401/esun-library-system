...
DROP FUNCTION IF EXISTS fn_list_books(BIGINT);
...
-- 查詢書籍與庫存列表
--
-- p_user_id 用於判斷目前登入者是否為該書借閱者。
-- 若未登入，p_user_id 可為 NULL，borrowed_by_me 會回傳 false。
-- =========================================================
...
--
-- 重點：
-- 1. 使用 FOR UPDATE 鎖定該筆 inventory，降低併發問題
-- 2. 確認書籍狀態為 AVAILABLE
-- 3. 更新 inventory 狀態為 BORROWED
-- 4. 新增 borrowing_record
--
-- Transaction 邊界由 Spring Service 層的 @Transactional 管理。
-- 此 Procedure 內部不撰寫 COMMIT / ROLLBACK，
-- 避免與 Spring Transaction 產生衝突。
-- =========================================================
